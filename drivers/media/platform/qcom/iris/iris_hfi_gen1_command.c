...
static int iris_hfi_gen1_queue_input_buffer(struct iris_inst *inst, struct iris_buffer *buf)
{
	...
	if (inst->domain == DECODER) {
		...
		return iris_hfi_queue_cmd_write(inst->core, &com_ip_pkt, com_ip_pkt.shdr.hdr.size);
	} else {
		...
		return iris_hfi_queue_cmd_write(inst->core, &uncom_ip_pkt, uncom_ip_pkt.shdr.hdr.size);
	}
}
...
static int iris_hfi_gen1_set_stride(struct iris_inst *inst, u32 plane)
{
	...
	plane_actual_info.plane_format[0].actual_stride = ALIGN(inst->fmt_src->fmt.pix_mp.width, 128);
	plane_actual_info.plane_format[0].actual_plane_buffer_height = ALIGN(inst->fmt_src->fmt.pix_mp.height, 32);
	plane_actual_info.plane_format[1].actual_stride = ALIGN(inst->fmt_src->fmt.pix_mp.width, 128);
	plane_actual_info.plane_format[1].actual_plane_buffer_height = (ALIGN(inst->fmt_src->fmt.pix_mp.height, 32)) / 2;

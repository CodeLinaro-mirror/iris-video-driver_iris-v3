					...
					u32 rc_type)
{
	...
	mbs_per_frame =
		(ALIGN(aligned_height, 16) * ALIGN(aligned_width, 16)) / 256;
...
static inline u32 hfi_buffer_bin_enc(u32 width, u32 height,
		u32 work_mode, u32 lcu_size, u32 num_vpp_pipes, u32 rc_type)
{
	...
	bitstream_size =
		size_bin_bitstream_enc(width, height, rc_type);
	...
	size = ALIGN(bitbin_size, 256) * total_bitbin_buffers + 512;
...
static u32 iris_vpu_enc_comv_size(struct iris_inst *inst)
{
	...
	if (inst->codec == V4L2_PIX_FMT_HEVC) {
		lcu_size = 32;
		return hfi_buffer_comv_enc(width, height, lcu_size, num_recon + 1, HFI_CODEC_ENCODE_HEVC);
	} else {
		lcu_size = 16;
		return hfi_buffer_comv_enc(width, height, lcu_size, num_recon + 1, HFI_CODEC_ENCODE_AVC);
	}
}
...
static u32 iris_vpu_enc_non_comv_size(struct iris_inst *inst)
{
	...
	if (inst->codec == V4L2_PIX_FMT_HEVC) {
		lcu_size = 32;
		return hfi_buffer_non_comv_enc(width, height, num_vpp_pipes, lcu_size, HFI_CODEC_ENCODE_HEVC);
	} else {
		lcu_size = 16;
		return hfi_buffer_non_comv_enc(width, height, num_vpp_pipes, lcu_size, HFI_CODEC_ENCODE_AVC);
	}
}
...
static u32 iris_vpu_enc_line_size(struct iris_inst *inst)
{
	...
	if (inst->codec == V4L2_PIX_FMT_HEVC) {
		lcu_size = 32;
		return hfi_buffer_line_enc(width, height, 0, num_vpp_pipes, lcu_size, HFI_CODEC_ENCODE_HEVC);
	} else {
		lcu_size = 16;
		return hfi_buffer_line_enc(width, height, 0, num_vpp_pipes, lcu_size, HFI_CODEC_ENCODE_AVC);
	}
}

...
static u32 size_av1d_lb_opb_wr1_nv12_ubwc(u32 frame_width, u32 frame_height)
{
	...
	size = (256 * ((y_width + 31) / 32 + (AV1D_MAX_TILE_COLS - 1)));
...
static u32 size_av1d_lb_opb_wr1_tp10_ubwc(u32 frame_width, u32 frame_height)
{
	...
	size = (256 * ((y_width + 47) / 48 + (AV1D_MAX_TILE_COLS - 1)));
					...
					bool is_opb, u32 num_vpp_pipes)
{
		...
		size = ALIGN(size_av1d_lb_fe_top_data(frame_width, frame_height),
			...
			ALIGN(size_av1d_lb_vsp_top(frame_width, frame_height),
				DMA_ALIGNMENT) +
			ALIGN(size_av1d_lb_recon_dma_metadata_wr(frame_width,
			frame_height), DMA_ALIGNMENT) * 2 +
...
static bool res_is_greater_than(u32 width, u32 height,
...
static u32 iris_vpu_dec_comv_size(struct iris_inst *inst)
{
	...
	/* 
	 * Todo: ask and update
	 * num_comv = (inst->fw_min_count > MIN_BUFFERS) ?
...
static int non_comv_count(struct iris_inst *inst)
{
	if (inst->codec == V4L2_PIX_FMT_AV1)
		return 0;
	else
